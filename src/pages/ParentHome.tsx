...
export function ParentHome() {
  ...
  const [pendingRequests, setPendingRequests] = useState<ParentConnectionRequest[]>([]);
  const [myStudents, setMyStudents] = useState<ParentConnection[]>([]);
  ...
  async function fetchConnections() {
    ...
    try {
      ...
      console.log('Fetching parent connections for:', currentUser.uid);
      
      // Fetch pending requests where parent is requestor OR requested
      ...
      // Fetch confirmed connections where parent is the parent
  ...
  // Fetch connections on mount and when currentUser changes
  useEffect(() => {
    fetchConnections();
  }, [currentUser]);
  ...
  async function handleStudentSearch(e?: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Only load reports for connected students (security check)
      ...
      // Add non-connected students without reports (for connection requests)
  ...
  async function handleRequestStudent(studentId: string, studentEmail: string) {
    ...
    try {
      ...
      const requestData = {
        ...
        status: 'pending',
        requestorRole: 'parent' as const,
        createdAt: serverTimestamp()
      };
      
      console.log('Creating parent request:', requestData);
      const docRef = await addDoc(collection(db, 'parentConnectionRequests'), requestData);
  ...
  async function loadStudentReports(studentUid: string, studentEmail?: string) {
    try {
      ...
      // Check if parent is connected to this student
      if (!currentUser) {
                ...
                <div className="reports-grid">
                  {selectedStudent.reports.map((report) => (
                    ...
                    >
                      ...
                      {/* Teacher/Parent-facing explanations */}
